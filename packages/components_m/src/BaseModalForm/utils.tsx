...
import { parse_form_item_name, format_form_item_name_and_label, get_form_item_title } from "src/utils";
...
function calLayout(targetLabelCol: number, percentStr: string,) {
    ...
    return { labelCol, wrapperCol: wrapperCol > 15 ? wrapperCol - 2 : wrapperCol, span }
}
        ...
        <Form.Item
            ...
            label={
                label ? (
                    ...
                    mchcEnv.isDev ? <span className={styles["form-item-label-unit"]}>({config?.id},{config.key})</span> : ''
