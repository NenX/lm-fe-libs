...
export class ScarredUterus extends Component {
  state = {
    treeToalc: null,
    toalcKeys: [],
  };
  ...
  async componentDidMount() {
    ...
    const treeToalc = await SMchc_TemplateTrees.getTemplateTree(33);
    const selecteToalc = await SMchc_TemplateTrees.findAlertAssessment(33, pregnancyId);
  ...
  handleOk = async () => {
    ...
    if (filterData.length == 0) {
      await SMchc_TemplateTrees.deleteHighrisk(dat);
      // await SMchc_TemplateTrees.saveAlertAssessment(dat);
    ...
    return (
      ...
      >
        ...
        <div style={{ display: 'none' }}>
          <div style={{ marginTop: '28px' }} ref={(refs) => (this.printTableRef = refs)} className={styles['printtoalc']}>
