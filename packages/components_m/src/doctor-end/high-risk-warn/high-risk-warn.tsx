...
export default function HighRiskWarn({
  ...
  const handleCheck = (checkedKeys, { checked, checkedNodes, node, event, halfCheckedKeys }) => {
  ...
  const handleCheckRemind = (checkedKeys, { checked, checkedNodes, node, event, halfCheckedKeys }) => {
    const newwebsocketData = cloneDeep(websocketData);
    const setlectDataobj = find(newwebsocketData, (item) => item.id == node.id);
    setSelectRemind([...selectRemind, setlectDataobj] as any);
  };
    ...
    return (
      <div>
        ...
        {tabBtn == handleType.process && treeData.children && (
          <>
            <span className="handle-time">{get(treeData, `handleTime`)?.format( 'YYYY-MM-DD')}</span>
        ...
        <div className="checkbox-content">
          ...
          >
            ...
            {tabBtn == handleType.process && (
              <>
                <span className="handle-time">{get(item, `handleTime`)?.format( 'YYYY-MM-DD')}</span>
                <CustomIcon type="icon-laber" className="icon-laber" />
              </>
            )}
