...
export default class MovePoint extends React.Component<IProps, IState> {
  ...
  onMouseUp2() {
    ...
    let oLeft = this.state.translateX;
    let oTop = this.state.translateY;
    let absoulteLeft = oLeft < (this.htmlWidth - this.bWidth) / 2 ? oLeft : this.htmlWith - oLeft;
    let absoulteTop = oTop < (this.htmlHeight - this.bHeight) / 2 ? oTop : this.himlHeight - oTop;
    if (absoulteLeft <= absoulteTop) {
      if (oLeft < (this.htmlWidth - this.bWidth) / 2) {
        oLeft = 0;
      } else {
        oLeft = this.htmlWidth - this.bWidth;
      ...
      if (oTop < (this.htmlHeight - this.bHeight) / 2) {
        oTop = 0;
      } else {
        oTop = this.htmlHeight - this.bHeight;
      }
      this.setState({ translateY: oTop });
    }
  }
