...
import {
  ...
  getSchedulingData,
  getYearStartEnd,
} from '@/pages/family-planning/booking-management/edit/util';
import { renderForm0_shared } from '@/pages/family-planning/tracing-management/list/components/table1/components/ModalForm';
import { valueToForm } from '@/pages/family-planning/tracing-management/list/components/table1/config/adapter';
import {
          ...
          <Col xs={8}>
            <Item label="手术名称" name="operationName" rules={requiredRule}>
              <Select<string>
                ...
                onChange={(target) => {
                  if (target) {
                    setTracingFormVisible(['人工流产', '药物流产'].includes(target) || mchcEnv.isDev);
                  }
                }}
              />
            </Item>
