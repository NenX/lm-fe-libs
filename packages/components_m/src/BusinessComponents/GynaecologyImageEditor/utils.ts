...
export interface IGynaecologyImageEditorProps {
...
export const GynaecologyImageEditorEvents = {
    save: 'GynaecologyImageEditor:save'
}
...
export function event_process(con: RealCanvas, __events: RealListenCbs) {
    ...
    return () => {
        con.off('drop', __events.drop)
            .off('mouse:down', __events['mouse:down'])
            .off('mouse:move', __events['mouse:move'])
            .off('mouse:up', __events['mouse:up'])
            .off('mouse:over', __events['mouse:over'])
            .off('mouse:out', __events['mouse:out'])
            .off('object:added', __events['object:added'])
    }
}