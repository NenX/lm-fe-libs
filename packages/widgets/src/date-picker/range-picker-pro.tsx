...
export type PeriodType = 'year' | 'month' | 'day' | 'hour' | 'minute' | 'second'
...
const RangePickerPro: React.FC<RangePickerProProps> = ({
    ...
    const handleRangeTime = (value: RangeValue | RangePickerProps['value'], periodTypeVal?: PeriodType) => {
        if (isArray(value) && value.length === 2 && periodTypeVal) {
            const times: RangeValue = [startOf(value[0], periodTypeVal), endOf(value[1], periodTypeVal)]
            setRangeTime(times)
    ...
    if (disabledSelect) {
        return <RangePicker locale={zh_CN} {...rangePickerOptions} />
    }

    return (
        <Space direction="horizontal" size={spaceSize}>
            <Radio.Group value={period} optionType={optionType} options={options} onChange={handlePeriodChange} />
            <RangePicker locale={zh_CN} {...rangePickerOptions} />
        </Space>
    )
}
