...
import { PeriodData } from './range-picker-pro'
...
export function getRangePickerProps({ periodType = 'minute', periodValue = 1 }: PeriodData): RangePickerProps {
...
type DateValue = moment.Moment | moment.Moment[] | string | string[] | number | number[] | undefined | null
...
export const defaultFormatDate = (text: any, format: any) => {
    if (!text) {
        return '-'
    }
